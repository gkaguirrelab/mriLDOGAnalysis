...
def second_level_run(first_level_results, template_path, left_pls_right_folder, left_min_right_folder, right_min_left_folder, output_folder):
    ...
    os.system('feat %s' % os.path.join(left_pls_right_folder,'group.fsf'))
    ...
    for i in sorted_list:
        ...
        main_replacements = {'GROUP_OUTPUT':left_min_right, 'TOTAL_VOLUME':len(os.listdir(first_level_results)),
                             ...
                             'HIGH_LEVEL_EV':full_higher_lvl_ev,
                             'GROUP_MEMBER':full_group_member} 
        with open('%s/main_template.fsf'%left_min_right_folder, 'r') as maininfile:
            with open('%s/group.fsf'%left_min_right_folder,'w') as mainoutfile:
                for line in maininfile:
                    for src, target in main_replacements.items():                        
                        line = line.replace(src, str(target))
                    mainoutfile.write(line)
                    
    os.system('feat %s' % os.path.join(left_min_right_folder,'group.fsf'))
    ...
    for i in sorted_list:
        ...
        main_replacements = {'GROUP_OUTPUT':right_min_left, 'TOTAL_VOLUME':len(os.listdir(first_level_results)),
                             ...
                             'HIGH_LEVEL_EV':full_higher_lvl_ev,
                             'GROUP_MEMBER':full_group_member} 
        with open('%s/main_template.fsf'%right_min_left_folder, 'r') as maininfile:
            with open('%s/group.fsf'%right_min_left_folder,'w') as mainoutfile:
                for line in maininfile:
                    for src, target in main_replacements.items():                        
                        line = line.replace(src, str(target))
                    mainoutfile.write(line)
    os.system('feat %s' % os.path.join(right_min_left_folder,'group.fsf'))