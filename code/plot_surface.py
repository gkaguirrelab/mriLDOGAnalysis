...
def plot_surface(subject_id, invivoTemp, path_to_R2_map, ldog_surface_and_calculations_folder, threshold, output):
    ...
    if 'V1' in subject_id:
        ...
        plotting.plot_surf_stat_map(loaded_inflated_left, loaded_lh_map, bg_map=sulc_map_lh,
                                    threshold=threshold, view='medial', figure=fig1, output_file=os.path.join(output,'%s_left_medial.png' % subject_id), vmax=1) 
        plotting.plot_surf_stat_map(loaded_inflated_right, loaded_rh_map, bg_map=sulc_map_rh,
                                    threshold=threshold, view='lateral', figure=fig4, output_file=os.path.join(output,'%s_right_lateral.png' % subject_id), vmax=1)     
